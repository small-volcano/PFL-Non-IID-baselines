...
def get_dataset(mode='train'):
    ...
    #    [transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))]) #Mnist

    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]) #CIFAR10

    dataset = CIFAR100(root='./data', train=True if mode=='train' else False, download=True, transform=transform)

    dataset.targets = sparse2coarse(dataset.targets) # cifar100  sparse2coarse

    n_sample = len(dataset.data)
    #SRC_N_CLASS = len(dataset.classes)
    SRC_N_CLASS = 10 # n_class
    # full batch
    ...
    data_by_class = rearrange_data_by_class( #这步将 target 以 int 的形式选出rearrange
...
def main():
    ...
    path_prefix = f'ou{args.n_user}c{args.n_class}-alpha{args.alpha}-ratio{args.sampling_ratio}'
